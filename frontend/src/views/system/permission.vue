...
const initParams = { pageNum: 1, pageSize: 10, permissionName: "" };
const params = ref({ ...initParams });
const initForm = { permissionName: "", permissionCode: "", description: "" };
...
const getTableData = async () => {
  const res = await http.get(`/permission/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&permissionName=${params.value.permissionName}`);
...
const delRow = async (id: number) => {
  const res = await http.get(`/permission/del?id=${id}`);
  if (res.code === 200) { ElMessage.success("删除成功"); await getTableData(); }
  else { ElMessage.error("删除失败"); }
};
const batchDel = async () => {
  if (multipleSelection.value.length === 0) { ElMessage.warning("请先选择要删除的权限"); return; }
  const res = await http.post("/permission/batchDel", selectedIds.value);
...
const submit = async () => {
  if (isEdit.value) {
    const res = await http.post(`/permission/edit?id=${form.value.id}`, form.value);
    if (res.code === 200) { ElMessage.success("编辑成功"); visible.value = false; await getTableData(); }
    else { ElMessage.error("编辑失败"); }
  } else {
    const res = await http.post("/permission/add", form.value);
  ...
  <div>
    <header class="flex">
      <div class="flex form-item">
        <p class="label-text">权限名：</p>
        <el-input v-model="params.permissionName" placeholder="请输入权限名" />
      </div>
    ...
    <main>
      <div class="flex op-box">
        <el-button v-if="hasPermission('permission', '新增')" :icon="Plus" type="primary" @click="handleAdd">新增</el-button>
        <el-button v-if="hasPermission('permission', '删除')" :icon="Delete" type="danger" :disabled="!hasSelected" @click="batchDel">批量删除</el-button>
      </div>
      <el-table :data="tableData" border @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" fixed="right">
          <template #default="scope">
            <el-button v-if="hasPermission('permission', '编辑')" type="primary" :icon="Edit" @click="handleEdit(scope.row)">编辑</el-button>
            <el-popconfirm v-if="hasPermission('permission', '删除')" title="您确定要删除吗?" @confirm="delRow(scope.row.id)">
              <template #reference>
    ...
    <el-dialog v-model="visible" :title="dialogTitle" width="500" :before-close="handleClose">
      <el-form :model="form" label-width="100">
        ...
        <el-form-item label="权限码" prop="permissionCode">
          <el-input v-model="form.permissionCode" placeholder="如: user:add" />
        </el-form-item>
