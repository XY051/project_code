...
const initParams = { pageNum: 1, pageSize: 10, roleName: "" };
const params = ref({ ...initParams });
const initForm = { roleName: "", roleCode: "", description: "" };
...
const getTableData = async () => {
  const res = await http.get(`/role/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&roleName=${params.value.roleName}`);
...
const delRow = async (id: number) => {
  const res = await http.get(`/role/del?id=${id}`);
  if (res.code === 200) { ElMessage.success("删除成功"); await getTableData(); }
  else { ElMessage.error("删除失败"); }
};
const batchDel = async () => {
  if (multipleSelection.value.length === 0) { ElMessage.warning("请先选择要删除的角色"); return; }
  const res = await http.post("/role/batchDel", selectedIds.value);
...
const submit = async () => {
  if (isEdit.value) {
    const res = await http.post(`/role/edit?id=${form.value.id}`, form.value);
    if (res.code === 200) { ElMessage.success("编辑成功"); visible.value = false; await getTableData(); }
    else { ElMessage.error("编辑失败"); }
  } else {
    const res = await http.post("/role/add", form.value);
  ...
  <div>
    ...
    <main>
      <div class="flex op-box">
        <el-button v-if="hasPermission('role', '新增')" :icon="Plus" type="primary" @click="handleAdd">新增</el-button>
        <el-button v-if="hasPermission('role', '删除')" :icon="Delete" type="danger" :disabled="!hasSelected" @click="batchDel">批量删除</el-button>
      </div>
      <el-table :data="tableData" border @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" />
        <el-table-column prop="roleName" label="角色名称" />
        <el-table-column prop="roleCode" label="角色标识码" width="150" />
        <el-table-column prop="description" label="描述" show-overflow-tooltip />
        <el-table-column prop="createTime" label="创建时间" width="180" />
        <el-table-column label="操作" fixed="right">
          <template #default="scope">
            <el-button v-if="hasPermission('role', '编辑')" type="primary" :icon="Edit" @click="handleEdit(scope.row)">编辑</el-button>
            <el-popconfirm v-if="hasPermission('role', '删除')" title="您确定要删除吗?" @confirm="delRow(scope.row.id)">
              <template #reference>
    ...
    <el-dialog v-model="visible" :title="dialogTitle" width="500" :before-close="handleClose">
      <el-form :model="form" label-width="100">
        ...
        <el-form-item label="角色标识码" prop="roleCode">
          <el-input v-model="form.roleCode" placeholder="请输入角色标识码" />
        </el-form-item>
