...
const initParams = { pageNum: 1, pageSize: 10, teacherId: null, studentId: null };
const params = ref({ ...initParams });
const initForm = { teacherId: null, studentId: null, trainingId: null, guidanceContent: "", guidanceTime: "" };
...
const getTableData = async () => {
  let url = `/teachingGuidance/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}`;
  if (params.value.teacherId) url += `&teacherId=${params.value.teacherId}`;
  if (params.value.studentId) url += `&studentId=${params.value.studentId}`;
...
const delRow = async (id: number) => {
  const res = await http.get(`/teachingGuidance/del?id=${id}`);
...
const batchDel = async () => {
  ...
  const res = await http.post("/teachingGuidance/batchDel", selectedIds.value);
...
const submit = async () => {
  if (isEdit.value) {
    const res = await http.post(`/teachingGuidance/edit?id=${form.value.id}`, form.value);
    if (res.code === 200) { ElMessage.success("编辑成功"); visible.value = false; await getTableData(); }
    else { ElMessage.error("编辑失败"); }
  } else {
    const res = await http.post("/teachingGuidance/add", form.value);
  ...
  <div>
    <header class="flex">
      <div class="flex form-item">
        <p class="label-text">教师ID：</p>
        <el-input v-model="params.teacherId" placeholder="教师ID" />
      </div>
      <div class="flex form-item">
        <p class="label-text">学生ID：</p>
        <el-input v-model="params.studentId" placeholder="学生ID" />
      </div>
    ...
    <main>
      <div class="flex op-box">
        <el-button v-if="hasPermission('guidance', '新增')" :icon="Plus" type="primary" @click="handleAdd">新增</el-button>
        <el-button v-if="hasPermission('guidance', '删除')" :icon="Delete" type="danger" :disabled="!hasSelected" @click="batchDel">批量删除</el-button>
      </div>
      <el-table :data="tableData" border @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" fixed="right">
          <template #default="scope">
            <el-button v-if="hasPermission('guidance', '编辑')" type="primary" :icon="Edit" @click="handleEdit(scope.row)">编辑</el-button>
            <el-popconfirm v-if="hasPermission('guidance', '删除')" title="您确定要删除吗?" @confirm="delRow(scope.row.id)">
              <template #reference>
    ...
    <el-dialog v-model="visible" :title="dialogTitle" width="600" :before-close="handleClose">
      <el-form :model="form" label-width="100">
        ...
        <el-form-item label="实训项目ID" prop="trainingId">
          <el-input-number v-model="form.trainingId" :min="1" placeholder="请输入实训项目ID" style="width: 100%" />
        </el-form-item>
        <el-form-item label="指导内容" prop="guidanceContent">
          <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 6 }" v-model="form.guidanceContent" placeholder="请输入指导内容" />
        </el-form-item>
