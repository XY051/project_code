  ...
  <div>
    <el-card style="margin-bottom: 15px">
      <el-form :inline="true" :model="searchForm">
        ...
        <el-form-item label="收藏类型">
          <el-select v-model="searchForm.itemType" placeholder="请选择收藏类型" clearable style="width: 160px">
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange" border stripe>
        ...
        <el-table-column prop="itemId" label="收藏项ID" width="120" />
        <el-table-column prop="itemType" label="收藏类型" min-width="150" />
...
function loadData() {
  http.get('/collect/page', { params: { pageNum: pageNum.value, pageSize: pageSize.value, ...searchForm } }).then((res: any) => {
...
function handleDel(id: number) {
  ElMessageBox.confirm('确定要删除该收藏记录吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.delete(`/collect/del?id=${id}`).then(() => {
...
function handleBatchDel() {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条数据吗？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.post('/collect/batchDel', selectedIds.value).then(() => {
