...
function loadData() {
  http.get('/ichType/page', { params: { pageNum: pageNum.value, pageSize: pageSize.value, ...searchForm } }).then((res: any) => {
...
function handleSubmit() {
  formRef.value?.validate((valid) => {
    if (!valid) return
    const apiCall = form.id ? http.put(`/ichType/edit?id=${form.id}`, form) : http.post('/ichType/add', form)
    apiCall.then(() => {
...
function handleDel(id: number) {
  ElMessageBox.confirm('确定要删除该资源分类吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.delete(`/ichType/del?id=${id}`).then(() => {
...
function handleBatchDel() {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条数据吗？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.post('/ichType/batchDel', selectedIds.value).then(() => {
