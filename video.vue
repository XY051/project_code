...
function loadData() {
  http.get('/video/page', { params: { pageNum: pageNum.value, pageSize: pageSize.value, ...searchForm } }).then((res: any) => {
...
function handleAdd() {
  ...
  Object.assign(form, { title: '', videoUrl: '', coverImage: '', description: '' })
...
function handleSubmit() {
  formRef.value?.validate((valid) => {
    if (!valid) return
    const apiCall = form.id ? http.put(`/video/edit?id=${form.id}`, form) : http.post('/video/add', form)
    apiCall.then(() => {
...
function handleDel(id: number) {
  ElMessageBox.confirm('确定要删除该视频吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.delete(`/video/del?id=${id}`).then(() => {
...
function handleBatchDel() {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条数据吗？`, '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    http.post('/video/batchDel', selectedIds.value).then(() => {
